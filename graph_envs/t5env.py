...
from t5 import convert_from_t5_format, convert_to_t5_format


class T5RLEnv(gym.Env):
    ...
    def __init__(self, args, sentences):
        ...
        self.gen_model = T5ForConditionalGeneration.from_pretrained(self.args.gen_model).to("cuda")
        self.gen_tokenizer = AutoTokenizer.from_pretrained("t5-base")
    ...
    def step(self, action):
        ...
        inputs, _, _ = convert_to_t5_format(self.nlp, [selected_sentence])
        try:
            masked_sentence = inputs[0]
        except IndexError:
            masked_sentence = "<extra_id_0>"
            pass
        logging.info(f"Masked sentence is {masked_sentence}")

        input_ids = self.gen_tokenizer(masked_sentence, return_tensors="pt", padding=True, truncation=True, max_length=128, add_special_tokens=True).input_ids.to("cuda")
        outputs = self.gen_model.generate(input_ids=input_ids, min_length=5, max_length=20, do_sample=True, top_k=50, top_p=0.99, num_return_sequences=15)
        
        gen_tokens = [self.gen_tokenizer.convert_ids_to_tokens(output) for output in outputs]
        gen_tokens = [[token for token in tokens if token not in ["<pad>", "<\s"]] for tokens in gen_tokens]
        gen_tokens = [[token for token in tokens if token not in string.punctuation] for tokens in gen_tokens]
        outputs = [self.gen_tokenizer.convert_tokens_to_string(tokens) for tokens in gen_tokens]
        gen_sentences = [convert_from_t5_format(masked_sentence, output) for output in outputs]
