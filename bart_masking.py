...
def mask_generate(args, sentence, gen_model, sim_model, tokenizer):
    masked_templates = []
    ...
    for masked_sentence in masked_sentences:
        ...
        masked_templates.append({"masked_sentence": masked_sentence, "mask_score": mask_score, "mask_standard_deviation": mask_std, "generated_sentences": output_sentences, 'original': sentence})

    masked_templates = sorted(masked_templates, key=lambda x: x["mask_score"], reverse=True)
    return masked_templates
...
def main(args):
    ...
    for sentence in tqdm(data):
        masked_templates = mask_generate(args, sentence, gen_model, sim_model, tokenizer)
        scored_sentences.append({"sentence": sentence, "masked_templates": masked_templates})
        all_masked_templates.extend(masked_templates)
    ...
    with open(args.output_file+'.txt', "w") as csvfile:
