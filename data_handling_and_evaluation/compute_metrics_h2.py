...
def main(args):
    ...
    for file_name in input_files:
        ...
        if file_name.endswith(".json"):
            ...
            with open(file_name, "r") as f:
                data = json.load(f)
                if not file_name.endswith("h2_bart.json"):
                    continue
                data = data["output_sentences"]
                for sent in data:
                    sent['gen_sentence_reward'] = float(sent['gen_sentence_reward'])
                    bleu_scores = [float(x) for x in sent['gen_sentence_logit'][1:-1].strip().split()]
                    sent['gen_sentence_logit'] = bleu_scores
                    if sent['gen_sentence_logit'][2] > 0.7 or sent['gen_sentence_reward'] > 0.7:
                        generated_sentences.append(sent)

    generated_sentences = sorted(generated_sentences, key=lambda sent: sent['gen_sentence_logit'][2]+sent['gen_sentence_reward'], reverse=True)
    ...
    with open(args.input_data+"/sorted_h2.json", "w") as f:
        json.dump(generated_sentences, f, indent=2)
