...
from graph_envs.sbartenv import SynthBARTRLEnv
from graph_envs.t5env import T5RLEnv


def main(args):
    os.environ["TOKENIZERS_PARALLELISM"] = "false"
    os.environ["SB3_LOGDIR"] = args.log_dir
    ...
    elif args.gen_model_name == "synth/facebook/bart-large":
        env = SynthBARTRLEnv(sentences=leakages, args=args)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_dir", type=str, default="/home/t-soadit/Projects/IEOutputs/", help="Log Directory")
    ...
    parser.add_argument('--use_sim_model', dest='use_sim_model', action='store_true', help="If set, we use paraphrase-based similarity model for reward.")
    ...
    args.rewards_log_file = os.path.join(args.log_dir, args.rewards_log_file)
    args.input_graph_file = os.path.join(args.log_dir, args.input_graph_file)
    args.output_graph_file = os.path.join(args.log_dir, args.output_graph_file)
