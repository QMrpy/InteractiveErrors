    ...
    try:
        ...
        for sentence in sentences:
            ...
            for i in range(len(ranked_word_ids)):
                ...
                sentence_copy[ranked_word_ids[i]] = "<extra_id_0>"
                ...
                outputs = mask_model.generate(input_ids, min_length=5, max_length=20, num_return_sequences=5, top_k=10, top_p=0.95, do_sample=True, no_repeat_ngram_size=2).detach().cpu().tolist()
                gen_tokens = [mask_tokenizer.decode(output, skip_special_tokens=True) for output in outputs]
                
                gen_sentences = []
                for j in gen_tokens:
                    x = copy.deepcopy(masked_sentence)
                    x[ranked_word_ids[i]] = j
                    x = " ".join(x)
                    gen_sentences.append(x)
        ...
        logging.warning("Code encountered some error. Exiting and saving outputs")
...
def main(args):    
    ...
    mask_tokenizer = T5Tokenizer.from_pretrained(args.mask_tokenizer)
    mask_model = T5ForConditionalGeneration.from_pretrained(args.mask_model).to('cuda')
...
if __name__ == "__main__":                                 
    ...
    parser.add_argument("--mask_tokenizer", type=str, default="t5-base", help="Tokenizer for masked language model")
    parser.add_argument("--mask_model", type=str, default="t5-base", help="Finetuned masked language model to use for word substitution")
