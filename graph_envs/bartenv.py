...
class BARTRLEnv(gym.Env):
    ...
    def __init__(self, args, sentences):
        ...
        self.gen_model = BartForConditionalGeneration.from_pretrained(self.args.gen_model).to("cuda")
        self.gen_tokenizer = AutoTokenizer.from_pretrained("facebook/bart-large")
        self.gen_tokenizer.pad_token = self.gen_tokenizer.eos_token

    def save_outputs(self):
        logging.info(f"Saving sentences and rewards to {self.args.output_file}")
        with open(os.path.join(self.args.log_dir, str(self.episode_id) + "_" + self.args.output_file), "w") as file:
    ...
    def step(self, action):
        ...
        masked_sentence, _ = convert_to_bart_mlm_format([selected_sentence])
        masked_sentence = masked_sentence[0]
        ...
        outputs = self.gen_model.generate(input_ids=input_ids, min_length=5, max_length=20, do_sample=True, top_k=100, top_p=0.99, num_return_sequences=20)
